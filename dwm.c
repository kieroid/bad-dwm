...
getstatus(int width)
{
	...
	#endif /* INVERSED */
	{
		...
		scheme[SchemeStatus] = drw_scm_create(drw, cols, alphas[0], 3);
...
setup(void)
{
	...
	setsignal(SIGCHLD, sighold); /* zombies */
	...
	for (i = 0; i < LENGTH(tagsel); i++)
		tagscheme[i] = drw_scm_create(drw, tagsel[i], alphas[0], 2);
	/* init bars */
	updatebars();
	updatestatus();
